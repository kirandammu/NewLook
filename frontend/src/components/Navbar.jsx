    ...
    return (
        <nav className="flex sticky top-0 items-center justify-between px-6 md:px-16 lg:px-20 py-3 border-b border-gray-300 bg-white z-30">
            ...
            <div className="hidden sm:flex items-center gap-8 uppercase">
                ...
                <NavLink to={'/'}  className={({isActive})=>`${isActive? 'border-b-2 text-[#0ee50e]' : ''}`} >Home</NavLink>
                <NavLink to={'/products'}  className={({isActive})=>`${isActive? 'border-b-2 text-[#0ee50e]' : ''}`} >All collections</NavLink>
                {user && <NavLink to={'/myorder'} className={({isActive})=>`${isActive? 'border-b-2 text-[#0ee50e]' : ''}`}>My Orders</NavLink>}
                <NavLink to={'/contact'} className={({isActive})=>`${isActive? 'border-b-2 text-[#0ee50e]' : ''}`}>Contact</NavLink>
                ...
                {user && (!seller?(<div onClick={()=>navigate('/cart')} className="relative cursor-pointer">
                    ...
                    <button className="absolute -top-2 -right-3 text-xs text-white bg-[#0ee50e] w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
                </div>):(<div onClick={()=>navigate('/seller')} className="relative cursor-pointer">
                    < MdAssignmentAdd  className='text-2xl font-bold'/>
                </div>))}
                {!user ? (<button onClick={()=>navigate('/login')} className="cursor-pointer px-8 py-2 bg-[#0ee50e] hover:bg-silver-600 transition text-white rounded-md">
                    Login
                </button>):(<button onClick={()=>logout()} className="cursor-pointer px-5 py-1.5 bg-[red] hover:bg-#0ee50e-600 transition text-white rounded-md">
                    Logout
                </button>)}
            </div>
