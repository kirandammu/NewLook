    ...
    return (<div className='fixed top-0 bottom-0 left-0 right-0 z-60 flex items-center bg-white'>
        <form onSubmit={submit} className="flex flex-col gap-4 m-auto items-start p-8 py-10 w-80 sm:w-[352px] rounded-lg shadow-xl border border-gray-200 bg-white">
            <div className='flex items-center mx-auto justify-center gap-2'>
              ...
              <div className='font-semibold text-xl uppercase text-[#0ee50e]'>User <span className='text-black'>{state === 'login' ?  'Login': 'Register'}</span></div>
            ...
            <button className="bg-[#0ee50e] hover:bg-green-500 transition-all text-white w-full py-2 rounded-md cursor-pointer">
                {state === "register" ? "Create Account" : "Login"}
            </button>
        <div onClick={()=>navigate('/')} className='flex items-center text-[#0ee50e] justify-center w-8 text-2xl mx-auto  cursor-pointer'><RxCrossCircled /></div>
        </form>
        </div>
    );
};
